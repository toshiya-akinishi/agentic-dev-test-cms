...
`pnpm dev` は既定で `127.0.0.1`（自分の PC からのみアクセス可）にバインドする。Next.js の `next dev` はデフォルトでは `0.0.0.0`（全ネットワークインターフェース）を待ち受け、同じ Wi-Fi/LAN 上の他端末からもアクセスできてしまうため、意図せず社内ネットワーク等に公開してしまわないよう明示的に制限している。

Expo アプリを**実機**（同一 LAN 上のスマホ）から接続してテストしたい場合など、意図的に LAN 公開したいときだけ:

```bash
pnpm dev:lan
```

を使う（信頼できるネットワーク上でのみ実行すること）。
