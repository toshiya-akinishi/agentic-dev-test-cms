...
export const seedNews = async (ctx: SeedCtx): Promise<void> => {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    await ctx.payload.create({
      ...
      data: {
        ...
        isPinned: i === 0,
        // 補-8-8-3: drafts 有効コレクションは _status 未指定だと 'draft' がデフォルトになるため明示
        _status: 'published',
      },
      overrideAccess: true,
      depth: 0,
  ...
  for (let i = 0; i < 5; i++) {
    ...
    // 補-1-12-1: 最後の1件は publishedAt を未来日時にし、_status: 'published' でも
    // 一覧 API からは除外される「予約公開」ケースを seed データとして残す（EP-16 の回帰テスト用）。
    const isScheduled = i === announcements.length - 1
    await ctx.payload.create({
      collection: 'news',
      data: {
        ...
        publishedAt: isScheduled ? iso(addDays(ctx.today, 7)) : iso(addDays(ctx.today, -i)),
        isPinned: false,
        _status: 'published',
      },
      overrideAccess: true,
